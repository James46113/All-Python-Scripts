...
def game():
    ...
    var = IntVar()
    Label(gc, text="GAME OVER!", font=("digital-7", 30), bg="black", fg="white").place(x=23, y=70)

    Button(gc, text="Play again", bg="black", fg="white", command=lambda: var.set(1)).place(x=113, y=160)
    if score1 > score2:
        Label(gc, text="Player 1 wins!", font=(None, 20), bg="black", fg="white").place(x=63, y=117)
    else:
        Label(gc, text="Player 2 wins!", font=(None, 20), bg="black", fg="white").place(x=63, y=117)
...
def keydown(e):
    ...
    if e.keysym == "l" and ball_x > 150:
        paddle2_y += 15
    if e.keysym == "Return":
        x = Button(gc, text="►", bg="black", fg="white", font=(None, 30), command=lambda: end_pause.set(1))  # ▶
        x.place(x=115, y=85)
...
if __name__ == "__main__":
    while play_again:
        ...
        score1 = 0
        score1_label = Label(gc, text="hi", font=("digital-7", 30), bg="black", fg="white")
        score1_label.place(x=100, y=5)
        score1_label['font'] = appHighlightFont
        
        score2 = 0
        score2_label = Label(gc, text="hi", font=("digital-7", 30), bg="black", fg="white")
        ...
        while not stop:
            ...
            gc.delete("all")
            gc.create_line(150, 0, 150, 250, dash=(6, 4), fill="white")
            gc.create_rectangle(paddle1_x, paddle1_y, paddle1_x + 10, paddle1_y + 50, fill="white")
            gc.create_rectangle(paddle2_x, paddle2_y, paddle2_x + 10, paddle2_y + 50, fill="white")
