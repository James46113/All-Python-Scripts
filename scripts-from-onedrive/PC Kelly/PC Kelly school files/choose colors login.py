...
def show_sign_up(num: int):
    ...
    if num == 0:
        ...
        window.geometry('{}x{}+{}+{}'.format(230, 290, window.winfo_screenwidth()//2-230//2, window.winfo_screenheight()//2-280//2))
...
window.config(bg=askcolor())

main_window = Toplevel()
main_window.withdraw()
main_window.title("PC Kelly")
main_window.geometry('{}x{}+{}+{}'.format(500, 300, window.winfo_screenwidth() // 2 - 500 // 2, window.winfo_screenheight() // 2 - 300 // 2))
...
sign_up_button = Button(window, text="Sign up", width=10, bg=askcolor(), command=lambda: add_user(username_input.get(), password_input.get()))
taken_label = Label(window, textvariable=taken_var, fg="red")

# Logo image
img = PhotoImage(file='Logo.png')
img = img.subsample(2)
img_label = Label(window, bg=askcolor(), image=img, bd=0)
...
error_label = Label(window, bg=askcolor(), textvariable=wrong, fg="red")
login = Button(window, bg=askcolor(), text="Login", width=10, command=get_input)
sign_up = Label(window, bg=askcolor(), text="Sign up")
sign_up.bind("<Button-1>", lambda e: sign())
back = Label(window, bg=askcolor(), text="Back")
back.bind("<Button-1>", lambda e: show_sign_up(1))

# Prompt labels
acount_info = Label(window, bg=askcolor(), text="Acount:")
personal_info = Label(window, bg=askcolor(), text="Address:")
...
street_input = Entry(window, bg=askcolor())
do_binds(street_input, "Street & house")

city_input = Entry(window, bg=askcolor())
do_binds(city_input, "City")

postcode_input = Entry(window, bg=askcolor())
do_binds(postcode_input, "Postcode")

email_input = Entry(window, bg=askcolor())
do_binds(email_input, "Email")


# Making the password and username inputs
username_input = Entry(window, bg=askcolor())
do_binds(username_input, "Username")

password_input = Entry(window, bg=askcolor())
